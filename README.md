...
| 场景 | 默认 guard 模式 |
| --- | --- |
| 明显的私钥上传，例如 curl 上传 `.ssh/id_rsa` | 本地规则直接拦截，不把该调用发给 Jev |
| 递归删除、丢弃 Git 修改、强制推送、凭据文件访问 | 确认这一次操作；没有交互界面时拦截 |
...
`warn` 不暂停原动作；提醒进入 Agent 上下文，最多每次用户请求四条。`hide` 隐藏整份结果（不是任意字符串替换）。自定义规则没有 `allow` 动作，因此不能覆盖另一项检查的拦截。多个自定义规则同时命中时，拦截优先，其次合并确认；自带检查仍独立生效。
